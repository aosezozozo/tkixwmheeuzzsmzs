...
- **`SYNC_COUNT`**: 控制**每个地区**最终要同步几个 IP 到 Cloudflare 的 DNS 记录（默认 `10` 个）。如果你设置了 6 个地区，单次运行总计会挑出 60 个极品节点分门别类推送。
- **`SCAN_COUNT`**: 控制每次生成多少个 IP 去抽卡测速（默认生成 `2000` 个，如果你使用自己的单 IP 精品库且数量很少，也可以把它改小）。

如果你想修改并发数量（默认 `200` 线程），可以在 `scripts/cf_scanner_sync.py` 的 `max_workers=200` 处修改。调高可加快测速，但需注意 API 接口的抗压能力。
