...
- **盲扫模式 (ALL)**：不知道填什么？直接填 `"ALL"`！系统会在全网疯狂盲扫，**只要碰到通的极品节点，不管属于哪些国家，凑够设定的数量（`ALL_MODE_LIMIT`）瞬间光速收网！** 然后自动识别地区并分类推送到你的域名。
- **纯净挖矿模式 (ALL_NOSYNC)**：新增加的模式！如果你只想收集优质节点和扩充高纯度 IP 段，不想频繁修改你的 DNS，直接填 `"ALL_NOSYNC"`！程序会像饿狼一样全网寻宝，并把扫到的神仙 IP 全部沉淀进你的 `ips-v4.txt` 和 `hot_cidrs.txt` 中！
...
- **`ALL_MODE_LIMIT`**: 专门用于控制 `ALL` 或 `ALL_NOSYNC` 全局盲扫模式下，总计要抓多少个优质 IP 就立刻停止（默认 `200` 个）。
